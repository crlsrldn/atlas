<script lang="ts">
  ...
  let { type } = $props<{ type: 'login' | 'signup' }>();
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      error = (err as Error).message || 'An error occurred during authentication.';
    } finally {
      loading = false;
    }
  }
...
<div class="glass-card p-8 max-w-md w-full mx-auto animate-fade-in-up">
  <div class="text-center mb-8">
    <div class="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-emerald-500/10 text-emerald-400 mb-4">
  ...
  <div class="mt-6 text-center text-sm text-zinc-400">
    {#if type === "signup"}
      Already have an account? <a href="/login" class="text-emerald-400 hover:text-emerald-300 font-medium transition-colors">Sign in</a>
    {:else}
      Don't have an account? <a href="/signup" class="text-emerald-400 hover:text-emerald-300 font-medium transition-colors">Sign up</a>
    {/if}
  </div>
</div>
